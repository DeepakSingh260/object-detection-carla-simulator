...
class EgoVehilce:
	...
	def __init__(self):
		...
		vehicle_bp = self.blueprint_library.filter('cybertruck')[0]
		spawnpoint = carla.Transform(carla.Location(x=-55.4,y=-55.0,z = 35),carla.Rotation(pitch = 0, yaw = 180, roll =0))
		self.vehicle = self.world.spawn_actor(vehicle_bp , spawnpoint)
		self.actor_list.append(self.vehicle)
		self.running = True
		
		self.control_vehicle = VehiclePIDController(self.vehicle , args_lateral={'K_P':1,'K_D':0.0,'K_I':0.0},args_longitudnal={'K_P':1,'K_D':0.0,'K_I':0.0})
		...
		self.camera.listen(lambda image : processImage(image))
	...
	def run(self):
		try:
			while self.running:
				...
				control_signal = self.control_vehicle.run_step(5,waypoint)
